...
for i = 1:numBootstraps
    ...
    [~, ~, ~, bootstrapAUC_bmode(i)] = perfcurve(bootstrapLabels, bootstrapScores_bmode, 1);
    [~, ~, ~, bootstrapAUC_slsc(i)] = perfcurve(bootstrapLabels, bootstrapScores_slsc, 1);
end

% Calculate 95% CI
ciLower_bmode = prctile(bootstrapAUC_bmode, 2.5);
ciUpper_bmode = prctile(bootstrapAUC_bmode, 97.5);
ciLower_slsc = prctile(bootstrapAUC_slsc, 2.5);
ciUpper_slsc = prctile(bootstrapAUC_slsc, 97.5);

% Display results
fprintf('AUC B-mode: %.3f\n', AUC_bmode);
fprintf('95%% Confidence Interval: [%.3f, %.3f]\n', ciLower_bmode, ciUpper_bmode);

fprintf('AUC SLSC: %.3f\n', AUC_slsc);
fprintf('95%% Confidence Interval: [%.3f, %.3f]\n', ciLower_slsc, ciUpper_slsc);